...
class AddHomeTeam extends Component {
  ...
  handleSubmit(e) {
    ...
    this.props.tide.actions.all.mutateAndTrack(
      [c.MATCH, c.HOMETEAM_FIRST_PLAYER_NAME],
      player1
    );
    this.props.tide.actions.all.mutateAndTrack(
      [c.MATCH, c.HOMETEAM_SECOND_PLAYER_NAME],
      player2
    );
    this.props.tide.actions.all.mutateAndTrack(
      [c.MATCH, c.HOMETEAM_COLOR],
      this.state.color
    );
    this.props.tide.actions.all.mutate(
      [c.MATCH, c.SHOW_COMPONENT],
      c.ADD_AWAYTEAM_COMPONENT
    );
  }
  ...
  render() {
    ...
    return (
      <div id="add-home-team">
        ...
        <InfoArea number="ℹ">
          The first thing we need to do is to add the name of the players to the
          home team
        </InfoArea>
        <InfoArea number="ℹ">
