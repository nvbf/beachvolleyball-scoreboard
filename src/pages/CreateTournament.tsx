...
const CreateTournament = () => {
  function parseAdminMatch(data: Match): AdminMatch {
    ...
    return {
      ...
      sets: [{ PointsHomeTeam: 0, PointsAwayTeam: 0, Number: 1 }],
      hasWinner: false,
      isFinalized: false,
      isStarted: false,
      scoreboardID: "",
  ...
  const handleFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e: any) => {
      ...
      const filteredMatches = rows
        .slice(1)
        .map((row: any[], index: number): Match | null => {
          if (isRowEmpty(row)) {
      ...
      setAdminMatches(
        (filteredMatches as Match[]).map((e) => parseAdminMatch(e)),
      );
    };
  ...
  const sendTournament = async (tournament: CustomTournament | null) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/sync/v1/custom/tournament/aa_test_csv`,
        {
          method: "POST",
          headers: {
