...
function formatMatch(match) {
	const team = match.homeTeam.player1 + '/' + match.homeTeam.player2 + ' - ' + match.awayTeam.player1 + '/' + match.awayTeam.player2;
	const score = '<td>' + match.sets[0][0] + '-' + match.sets[0][1] + '</td><td>' +
              match.sets[1][0] + '-' + match.sets[1][1] + '</td><td>' +
              match.sets[2][0] + '-' + match.sets[2][1] + '</td>';
	const newMatchNode = '<tr id="match-' + match.id + '"><td>' + team + '</td>' + score;
	const matchNode = $('#match-' + match.id);
